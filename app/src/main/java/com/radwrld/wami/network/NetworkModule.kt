...
data class MessageHistoryItem(@SerializedName("id") val id: String, @SerializedName("jid") val jid: String, @SerializedName("text") val text: String?, @SerializedName("type") val type: String?, @SerializedName("isOutgoing") val isOutgoing: Int, @SerializedName("status") val status: String, @SerializedName("timestamp") val timestamp: Long, @SerializedName("name") val name: String?, @SerializedName("media_url") val mediaUrl: String?, @SerializedName("mimetype") val mimetype: String?, @SerializedName("fileName") val fileName: String?, @SerializedName("quoted_message_id") val quotedMessageId: String?, @SerializedName("quoted_message_text") val quotedMessageText: String?, @SerializedName("reactions") val reactions: Map<String, Int>?, @SerializedName("media_sha256") val mediaSha256: String?)
fun MessageHistoryItem.toDomain(): Message = Message(id = this.id, jid = this.jid, text = this.text, isOutgoing = this.isOutgoing > 0, type = this.type, status = this.status, timestamp = this.timestamp, name = this.name, senderName = this.name, mediaUrl = this.mediaUrl, mimetype = this.mimetype, fileName = this.fileName, quotedMessageId = this.quotedMessageId, quotedMessageText = this.quotedMessageText, reactions = this.reactions ?: emptyMap(), mediaSha256 = this.mediaSha256)
data class Conversation(@SerializedName("jid") val jid: String, @SerializedName("name") val name: String?, @SerializedName("is_group") val isGroupInt: Int = 0, @SerializedName("last_message") val lastMessage: String?, @SerializedName("last_message_timestamp") val lastMessageTimestamp: Long?, @SerializedName("unreadCount") val unreadCount: Int?) {
...
object ApiClient {
    ...
    private fun createOkHttpClient(context: Context, useLongTimeouts: Boolean): OkHttpClient {
        ...
        return OkHttpClient.Builder()
            .addInterceptor(HttpLoggingInterceptor().apply { level = if (useLongTimeouts) HttpLoggingInterceptor.Level.NONE else HttpLoggingInterceptor.Level.BODY })
            ...
            .apply { if (useLongTimeouts) { readTimeout(5, TimeUnit.MINUTES); writeTimeout(5, TimeUnit.MINUTES) } }
            .build()
    }
