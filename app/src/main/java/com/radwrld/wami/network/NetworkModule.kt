...
object ApiClient {
    ...
    fun resolveUrl(context: Context, path: String?): String? {
        if (path.isNullOrBlank() || path.startsWith("http")) return path
        val serverUrl = getBaseUrl(context).removeSuffix("/")
        return "$serverUrl$path"
    }

    fun resolveAvatarUrl(context: Context, jid: String?): String? {
        if (jid.isNullOrBlank()) return null
        val serverUrl = getBaseUrl(context).removeSuffix("/")
        return "$serverUrl/avatar/$jid"
    }
...
object SyncManager {
    ...
    private fun registerListeners() = socket?.apply {
        ...
        on(Socket.EVENT_CONNECT_ERROR) {
            ...
            if (error.contains("Invalid session ID")) {
                _authError.value = AuthError.INVALID_SESSION
                disconnect()
            } else {
                scheduleReconnect()
            }
    ...
    private fun scheduleReconnect() {
        if (!isInitialized.get() || _authError.value != null) return
        retryCount++
    ...
    private suspend fun handleIncomingStatus(json: String) {
        runCatching {
            ...
            val statuses = items.map {
                it.copy(mediaUrl = ApiClient.resolveUrl(appContext, it.mediaUrl))
            }
...
class SyncService : Service() {
    companion object {
        ...
        private const val CHANNEL_NAME = "Estado de Conexión"
    }
